...
extension GraphQLConnectionScenario3Tests {
    ...
    // Create a post, and two comments associated with the same post.
    // Retrieve the post, and access the `List` of comments via `fetch` which performs a load from the API
    // Once the comments are fetched, check if there are more items to load and perform pagination
    // Expect that the number of comments in exactly two.
